...
if __name__=="__main__":
    ...
    cfg.cmcf.step_size=0.25        # use smaller step size for thin filopodia (default 0.5) 
    cfg.cmcf.solver = 'pardiso'    # or 'scipy' without MKL

    # since the filo is very thin we need to smooth the MCF deformation to minimize singularity that makes height of the filo be under-estimated.
    cfg.cmcf.extra_smooth = True
    cfg.cmcf.deltaL_smooth = 5e-3 # this makes a big difference ! for filo 
    # additionally smooth the signed distance function field to improve gradient flow    
    cfg.cmcf.smooth = 3.0           # Gaussian sigma on binary mask before SDF (default 1.0)
    cfg.cmcf.smooth_gradient = 3.0 
    ...
    meshfile = '../example_data/synthetic_data/filo/Cell200/mesh_gt_protrusion_color.obj'
    saveoout = os.path.join(r'D:\Work\Projects\Danuser\u-Protrude3D_paper', 
                            'example_data/synthetic_data/filo/Cell200/',
    ...
    """
    gt_dir = '../example_data/synthetic_data/filo/Cell200'
    pred_dir = saveoout
